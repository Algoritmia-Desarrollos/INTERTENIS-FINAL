...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (const date of sortedDates) {
        ...
        finalHtml += `<h2 class="text-2xl font-bold text-gray-800 mb-4 capitalize border-b pb-2">${formattedDate}</h2>`;

        for (const sede in sedes) {
            finalHtml += `<h3 class="text-xl font-semibold text-gray-700 mt-6 mb-3">Sede: ${sede}</h3>`;
            finalHtml += `
            ...
            sedes[sede].forEach(match => {
                ...
                finalHtml += `
                    <tr>
                        ...
                        <td class="px-3 py-3 text-sm whitespace-nowrap">${match.time || ''}</td>
                        <td class="px-3 py-3 text-sm whitespace-nowrap text-right ${p1_class}">${match.player1.name}</td>
                        ...
                        <td class="px-3 py-3 text-sm whitespace-nowrap text-left ${p2_class}">${match.player2.name}</td>
                        <td class="px-3 py-3 text-sm whitespace-nowrap text-gray-500">${match.category}</td>
                    </tr>
                `;
            });
